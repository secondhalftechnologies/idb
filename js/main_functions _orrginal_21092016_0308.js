...
function checkEmailUser(email_txt,cust_email,req_page,email_error_span)
{
			...
			if(typeof(cust_session) == 'undefined')
			{
				cust_session	= '';	
			}
		...
		else
		{
			...
			$.ajax({
				...
				success: function(response) 
				{
					...
					{
						//setTimeout("",10000);
						//window.location.href="/";				// For Server Use Only
						window.location.href="/planeteducate";	// For Localhost Only
					} 
					else 
					{
						loading_hide();
...
function placeOrder()
{
		...
		{
			var sendInfo 	= {"cust_session":cust_session,"add_id":add_id,"payment_mode":payment_mode,"ord_comment":ord_comment,"placeOrder":1}
