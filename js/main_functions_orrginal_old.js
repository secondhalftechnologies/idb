...
$('#user_register').on('submit', function(e) 
{
			...
			if ($('#user_register').valid())
			{
				...
				else
				{
					...
					var sendInfo 		= {"cust_fname":cust_fname,"cust_lname":cust_lname,"cust_email":cust_email,"cust_mobile_num":cust_mobile_num,"cust_address":cust_address,"cust_country":cust_country,"cust_state":cust_state,"cust_city":cust_city,"cust_pincode":cust_pincode,"cust_password":cust_password,"cli_browser_info":cli_browser_info,"cli_ip_address":cli_ip_address,"user_register":1}
...
function placeOrder()
{
		...
		var pay_online_mode		= 0;
		if(payment_mode)
		{
			pay_online_mode		= $("input[name=pay_online_mode]:checked").val();
		}
		...
		{
			var sendInfo 	= {"cust_session":cust_session,"pay_online_mode":pay_online_mode,"add_id":add_id,"payment_mode":payment_mode,"ord_comment":ord_comment,"placeOrder":1}
				...
				$.ajax({
					...
					success: function(response) 
					{
						...
						if(data.Success == "Success") 
						{
							...
							if(sPage == "page-profile.php")
							{
								$("#new-address-block").slideUp();
								$("#address_data").slideDown();
								getAddressData('address_data');															
							}
							else
							{
								$("#new-address-block").slideUp();
...
function sendVerificationCode(req_type)
{
			loading_show();
			var cust_session		= $("#cust_session").val();
			if(cust_session == "")
			...
			else
			{
				...
				var sendInfo 	= {"cust_session":cust_session,"req_type":req_type,"send_code":1}
				var verify_code	= JSON.stringify(sendInfo);				
				$.ajax({
						url: base_url+"includes/main.php",
						type: "POST",
						data: verify_code,
...
function addressSelect(address_id)
{
			...
			$('.my-address-box').each(function () 
			{
				if(this.id == address_id)
				{
					...
					$("#"+address_id).html('<i class="fa fa-check-square-o"></i>&nbsp;Deliver to this address');
				}
				else
				{
					if($("#"+this.id).hasClass('alt')) 
					...
					else
					{
						...
						$("#"+this.id).html('<i class="fa fa-square-o"></i>&nbsp;Deliver to this address');						
					}
				}
			});
		}
