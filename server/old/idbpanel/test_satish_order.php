...
function sendOrderEmailVendor($ord_id,$type)
{
	...
	$sql_get_vendor_info		= " SELECT distinct org_id,org_name,org_primary_email,org_secondary_email,org_primary_phone,org_secondary_phone,ord_id_show,ord_created FROM `tbl_oraganisation_master` tom,`tbl_products_master` tpm,`tbl_cart` tc,`tbl_order` tord ";
    ...
    $res_get_fs_data	= mysqli_query($db_con, $sql_get_fs_data) or die(mysqli_error($db_con));
			...
			sendEmail('satishdhere007@gmail.com',$subject,$message,'Order Mail Backend ('.$uid.')');
			/*if(trim($vendor_secondary_email) != "")
			{
				sendEmail('vendor email(Backend)',$vendor_secondary_email,$subject,$message);
			}
			if(trim($vendor_tertiary_email) != "")
			{
				sendEmail('vendor email(Backend)',$vendor_tertiary_email,$subject,$message);
			}
			sendEmail('vendor email(Backend)','cynthia@planeteducate.com',$subject,$message);
			sendEmail('vendor email(Backend)','prog3.php@edmission.in',$subject,$message);			
			sendEmail('vendor email(Backend)','support@planeteducate.com',$subject,$message);*/
		}
	}
	return true;
	/*Select last Order and Details*/	
...
*/	$message_body 					.= '<tr>';
	...
	//return $message;
	//$mail_response 					= sendEmail("Order Mail (Backend)",$cust_email,$subject,$message);
	$demo = sendEmail("satishdhere007@gmail.com",$subject,$message);
	echo $demo;
	return $message;
	if(sendEmail("Order Mail (Backend)",'satishdhere007@gmail.com',$subject,$message))
	{   
		//sendEmail("Order Mail (Backend)",'support@planeteducate.com',$subject,$message);
	    return true;
	}
	else
	{
...
if((isset($obj->test_satish_order)) == "1" && isset($obj->test_satish_order))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		$sql_load_data  				 = " SELECT tcm.*,tos.*,tc.cust_lname,tc.cust_fname FROM `tbl_order` as tos ";
	   ...
	   //$sql_load_data  				.= " INNER JOIN tbl_cart as tcs ON tos.ord_custid = tcs.cart_custid ";
		...
		{		
			...
			$order_data 				.= '<th style="text-align:center">Comment</th>';
          	$order_data 				.= '</tr>';
      		$order_data 				.= '</thead>';
      		$order_data 				.= '<tbody>';
		   ...
		   $res_get_fs_data	= mysqli_query($db_con, $sql_get_fs_data) or die(mysqli_error($db_con));
			...
			while($row_get_fs_data = mysqli_fetch_array($res_get_fs_data))
			{
				...
				$order_data 		.= '<td>';
			    $order_data 		.= '<textarea name="comment_'.$row_load_data['ord_id'].'" id="comment_'.$row_load_data['ord_id'].'">'.$row_load_data['ord_admin_comment'].'</textarea><br>';
			    $order_data 		.= '<input value="Comment" id="'.$row_load_data['ord_id'].'" class="btn-success" onclick="comments(this.id);" type="button"></td>';
