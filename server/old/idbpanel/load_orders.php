...
function sendOrderEmailVendor($ord_id,$type)
{
	...
	$sql_get_vendor_info		= " SELECT distinct org_id,org_name,org_primary_email,org_secondary_email,org_primary_phone ";
	$sql_get_vendor_info		.= " ,org_secondary_phone,ord_created,ord_id_show FROM `tbl_oraganisation_master` tom,`tbl_products_master` tpm,`tbl_cart` tc,`tbl_order` tord ";
		...
		while($row_get_vendor_info	= mysqli_fetch_array($result_get_vendor_info))
		{
			...
			while($row_get_fs_data = mysqli_fetch_array($res_get_fs_data))
			{
				if($row_get_fs_data['fs_type_value'] == 0)
				{
					...
					{
						$cart_data		= '<td><span style="color:f00;">Free Shipping</span></td></tr>';
			...
			sendEmail($vendor_primary_email,$subject,$message,'Ventor Mail From Backend  ('.$uid.') ');
			if(trim($vendor_secondary_email) != "")
			{
				sendEmail($vendor_secondary_email,$subject,$message,'Vendor Mail From Backend  ('.$uid.') ');
			}
			if(trim($vendor_tertiary_email) != "")
			{
				sendEmail($vendor_tertiary_email,$subject,$message,'Vendor Mail From Backend  ('.$uid.') ');
			}
			sendEmail('cynthia@planeteducate.com',$subject,$message,'Vendor Mail From Backend  ('.$uid.') ');
			sendEmail('prog3.php@edmission.in',$subject,$message,'Vendor Mail From Backend  ('.$uid.') ');			
			sendEmail('support@planeteducate.com',$subject,$message,'Vendor Mail From Backend  ('.$uid.') ');
		}
	}
	return true;
	/*Select last Order and Details*/	
...
/* Mail On Order*/
/* Mail On Order */
function sendOrderMail($ord_id,$type,$shipping_date)
{
	global $db_con;
	global $date;
	global $min_order_value;
	...
	if($type == 1)	
	{
		$sql_get_sub_order_status 		= " SELECT orstat_name,orstat_id FROM `tbl_cart` tc INNER JOIN `tbl_order_status` as tos ON tos.orstat_id = tc.`cart_status` where tc.`cart_id` = '".$ord_id."' "; // this is cart it i.e. sub order id
		...
		$orstat_id                      = 	$row_get_sub_order_status['orstat_id'];		
	}
	elseif($type == 0)
	{
		$order_status			= ucwords($row_order_details['orstat_name']);	
		$orstat_id              = 	$row_order_details['ord_pay_status'];
	}
	...
	while($row_get_cart_details = mysqli_fetch_array($result_get_cart_details))
	{
		...
		$result_get_product_details = mysqli_query($db_con,$sql_get_product_details) or die(mysqli_error($db_con));
		$row_get_product_details 	= mysqli_fetch_array($result_get_product_details);
		$imagepath 	= "images/planet/org".$row_get_product_details['prod_orgid']."/prod_id_".$row_get_product_details['prod_id']."/small/".$row_get_product_details['prod_image_name'];
		$product_name				= ucwords($row_get_product_details['prod_name']);
		if($orstat_id !=7)// Not delievery
		{		
			$order_data					.= '<tr>';
			$order_data					.= '<td align="center" valign="top" width="30%">';
			$order_data					.= '<img src="'.$prod_imagepath.'" alt="'.$product_name.'">';
	...
	if($orstat_id !=7)// Not delievery
	{
			...
			while($row_get_fs_data = mysqli_fetch_array($res_get_fs_data))
			{
				...
				{   
					if(($row_get_fs_data['fs_start_price'] <= $cart_total) && ($cart_total <= $row_get_fs_data['fs_end_price']))
					{
						$order_data 	.='<td><span style="color:f00;">'. $row_get_fs_data['fs_type_value'].'</span></td></tr>';
						$shipping_charge =$row_get_fs_data['fs_type_value'];
					}
				}
			}
	...
	$message_body 					.= '<tr>';
	$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
	//$message_body 					.= ' style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
	$message_body 					.= ' align="left">Dear, '.ucwords($cust_fname).'<br><br></td>';		
  //  echo json_encode(array("Success"=>"fail","resp"=>$orstat_id));exit();
	$message_body 					.= '</tr>';
	if($orstat_id ==6)// order is shipped
	{
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= ' style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
		$message_body 					.= ' align="left">We thought you\'d like to know that we\'ve dispatched your item(s). Your order is on the way. <br><br></td>';		
		
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= 'style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif;';
		$message_body 					.= ' align="left">Your order will reach you by '.$shipping_date.' <br><br></td>';		
		$message_body 					.= '</tr>';
	
	}
	elseif($orstat_id==4)// order is processed
	{
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= ' style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
		$message_body 					.= ' align="left">This is to let you know that we\'ve processed your order. Your order is on the way.<br><br></td>';		
		$message_body 					.= '</tr>';
	}
	elseif($orstat_id==7)// order is delievered
	{
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= ' style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
		$message_body 					.= ' align="left">This is to inform you that our delivery executive has delivered a package from your order ID '.$ord_id_show.'.
...
<br><br></td>';		
		...
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= 'style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
		$message_body 					.= 'align="left"><b>
The order was delivered to : </b>
<br><br></td>';		
		$message_body 					.= '</tr>';
	}
	elseif($orstat_id==8)// order is Cancelled
	{
		
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Title" data-size="Title" data-min="10" data-max="30" ';
		//$message_body 					.= ' style="font-size:14px; color:#494949; font-family:\'Open Sans\', sans-serif; ';
		$message_body 					.= ' align="left">This is to inform you that this order has been cancelled. We have initiated your refund of Rs. '.$order_total.'. You will receive your refund within 7-10 working days.
...
<br><br></td>';		
	...
	{
		$message_body 					.= '<tr>';
		$message_body 					.= '<td data-color="Name" data-size="Name" align="left"> Order Summary:<br>Your Order ID : '.$ord_id_show.'<br>Purchased on  : '.$order_created.'<br><br> ';
		$message_body 					.= '</td>';
		$message_body 					.= '</tr>';			
		$message_body 					.= '<tr>';
	}
	$message_body 					.= '<td data-color="Name" data-size="Name" align="left"> ';
	if($orstat_id !=7 && $orstat_id !=8)
	{
		$message_body 					.= ' Shipping Details: <br> ';
	}
	if($orstat_id !=8)
	{
		$message_body 					.= ' '.ucwords($cust_fname." ".$cust_lname).':<br> '.$cust_details_address.',<br>'.$cust_pincode.'<br>'.$cust_city.'<br>'.$cust_state.'<br><br> ';
		$message_body 					.= '</td>';
		$message_body 					.= '</tr>';	
	}
	...
	if($orstat_id ==7)
	{
		$message_body 					.= '<tr><td align="left"><b>Package Contents:<b></td></tr>';									
		$message_body 					.= '<tr>';
	}
	$message_body 					.= '<td data-color="Name" data-size="Name" align="left">'.$order_data;
	...
	$message_body 					.= '</tr>';
	$message_body 					.= '<tr><td data-bgcolor="Line Color" height="1" width="520" bgcolor="#cedcce"></td></tr>';	
	$message_body 					.= '<tr><td data-bgcolor="Line Color" height="1" width="520" bgcolor="#cedcce">Please give us your valuable feedback <a href="https://www.surveymonkey.com/r/2JSJ358">here</a></td></tr>';	//14072017
	$message_body 					.= '<tr>';
	...
	$message 						= mail_template_header()."".$message_body."".mail_template_footer();
	//return 	$message;
	//$mail_response 	$cust_email				= sendEmail("Order Mail (Backend)",$cust_email,$subject,$message);
	 
	if(sendEmail($cust_email,$subject,$message,"Order Mail (Backend)".$uid))
	{
		if($cust_email !="satishdhere007@gmail.com")
		{
			sendEmail('support@planeteducate.com',$subject,$message,"Order Mail (Backend).".$uid);
		}
...
if((isset($obj->load_orders)) == "1" && isset($obj->load_orders))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		$sql_load_data  				 = " SELECT tcm.*,tos.*,tc.cust_lname,tc.cust_fname,cust_comment FROM `tbl_order` as tos ";
		//$sql_load_data  				.= " (SELECT `orstat_name` FROM `tbl_order_status` WHERE `orstat_id` = ord_status) as  order_status FROM `tbl_order` as tos ";
		$sql_load_data  				.= " INNER JOIN tbl_order_status as tos1 ON tos.ord_pay_status = tos1.orstat_id  ";
	   ...
	   //$sql_load_data  				.= " INNER JOIN tbl_cart as tcs ON tos.ord_custid = tcs.cart_custid ";
		...
		///////done by satish ///////////////////////////////////////////
		//$sql_load_data 					.= " WHERE 1=1 AND tcm.cart_type = 'complete' AND tos.ord_pay_status != '0' ";
		$sql_load_data 					.= " WHERE 1=1 AND  tos.ord_pay_status != '0' ";
		
		if($order_status!="")
		{
			$sql_load_data 					.= " AND  ord_pay_status = '".$order_status."' ";
		}
		...
		if($daterange !="All")
		{
			...
			{   
				...
				$start_date =$last_year.'-'.$last_month.'-01';
				$end_date   =$last_year.'-'.$last_month.'-'.$days;
				$sql_load_data 					.= " AND  ord_created > '".$start_date." 00:00:00' AND  ord_created < '".$end_date." 00:00:00' ";
			}
			if($daterange =="specific_date")
			{
				$datentime  = explode(' ',$start_date);
				
				$sql_load_data 					.= " AND  ord_created > '".$start_date." 00:00:00' AND  ord_created < '".$start_date." 23:59:00' ";
			}
			if($daterange =="date_range")
			{
				//$date = date('Y-m-d',strtotime("-1 months"));
				$sql_load_data 					.= " AND  ord_created > '".$start_date." 00:00:00' AND  ord_created < '".$end_date."' ";
			}
		...
		if($search_text != "")
		{
			...
			$sql_load_data .= " AND (ord_id like '%".$search_text."%' or ord_total like '%".$search_text."%' or ord_pay_online_mode like '%".$search_text."%'  ";
		...
		{		
			...
			$order_data 				.= '<th style="text-align:center">Total Amount(Shipping Charges)</th>';
			$order_data 				.= '<th style="text-align:center">View</th>';
			$order_data 				.= '<th style="text-align:center">Order Status</th>';
			$order_data 				.= '<th style="text-align:center">Comment / Feedback</th>';
          	$order_data 				.= '</tr>';
      		$order_data 				.= '</thead>';
      		$order_data 				.= '<tbody>';
			...
			while($row_load_data = mysqli_fetch_array($result_load_data))
			{
				...
				while($row_load_cart_data = mysqli_fetch_array($result_load_cart_data))
				{
					...
					$cart_discount      +=$row_load_cart_data['cart_discount'];
				}
				...
				$order_data 			.= '<div><b>Payment method:</b>'.$row_load_data['ord_pay_type'];
				if($row_load_data['ord_pay_type']=="Pay Online")
				{
					$order_data 			.= '('.$row_load_data['ord_pay_online_mode'].')';
				}
				$order_data 			.= '</div>';
				...
				$order_date = strtotime($row_load_data['ord_created']);
				
				$order_data				.='<td>'.date(' j F, Y',$order_date).'</td>';
				
				
				
			 //$cart_total =$row_load_data['ord_total'] -$row_load_data['ord_shipping_charges'] ;
				$order_data				.='<td>'.$row_load_data['ord_total'];
				
				
				
			//	$order_data .=$cart_total;//echo json_encode($cart_total);
		   ...
		   $res_get_fs_data	= mysqli_query($db_con, $sql_get_fs_data) or die(mysqli_error($db_con));
			...
			while($row_get_fs_data = mysqli_fetch_array($res_get_fs_data))
			{
				...
				$order_data 		.= '<div class="center-text" style="text-align: center;margin: 0px auto;width: 82%;';
				if($row_load_data['ord_pay_status'] !='6')
				{
					$order_data .=' display:none';
				}
				$order_data 		.= ' ">';
				
				$order_data			.= '<input id="ord_date'.$row_load_data['ord_id'].'" name="s" class="center-text datepicker1 input-small " data-rule-required="true" style="display: block; height: 22px; width: 100%;" type="text" placeholder="Sipping Date">';
				$order_data 		.= '</div><br>';	
				...
				$order_data             .=' $("#ord_date'.$row_load_data['ord_id'].'").datepicker({
				changeMonth	: true,
				changeYear	: true,
				dateFormat	: \'dd-mm-yy\',
				...
				$order_data 		.= '<td>';
			    $order_data 		.= '<textarea name="comment_'.$row_load_data['ord_id'].'" id="comment_'.$row_load_data['ord_id'].'" onchange="comments('.$row_load_data['ord_custid'].',this.value);" >'.$row_load_data['cust_comment'].'</textarea><br>';
			    
					
				$order_data 		.= '<textarea name="feedback_'.$row_load_data['ord_id'].'" id="feedback_'.$row_load_data['ord_id'].'" onchange="feedback('.$row_load_data['ord_id'].',this.value);" >'.$row_load_data['feedback'].'</textarea><br>';
				...
				$sql_get_sub_orders 	.= " from tbl_cart tc INNER JOIN tbl_products_master tpm ON tc.cart_prodid=tpm.prod_id where cart_orderid = '".$row_load_data['ord_id']."' ";
				...
				while($row_get_sub_orders 	= mysqli_fetch_array($result_get_sub_orders))
				{
					...
					$order_data 		.= '<div class="center-text" style="text-align: center;margin: 0px auto;width: 82%;';
					if($row_get_sub_orders['cart_status'] !='6')
					{
						$order_data .=' display:none';
					}
					$order_data 		.= ' " >';
					
					$order_data			.= '<input id="cart_ord_date'.$row_get_sub_orders['cart_id'].'" name="s" class="center-text datepicker1 input-small " data-rule-required="true" style="display: block; height: 22px; width: 100%;"  placeholder="Shipping Date" type="text">';
					$order_data 		.= '</div><br>';	
					...
					$order_data         .=' $("#cart_ord_date'.$row_get_sub_orders['cart_id'].'").datepicker({
   		changeMonth	: true,
		changeYear	: true,
		dateFormat	: \'dd-mm-yy\',
		yearRange 	: \'c:c\',//replaced "c+0" with c (for showing years till current year)
...
if((isset($obj->send_Order_mail)) == "1" && isset($obj->send_Order_mail))
{
	...
	if($type_id != "")
	{
		if($towhom == "vendor")
		{
			if(sendOrderEmailVendor($type_id,$type,$shipping_date))
			{
		...
		{
		    /*$response_array = array("Success"=>"fail","resp"=>sendOrderMail($type_id,$type,$shipping_date)."Mail Sent to Vendor");
			echo json_encode($response_array);
			exit(0);*/		
			if(sendOrderMail($type_id,$type,$shipping_date))
			{
				$response_array = array("Success"=>"Success","resp"=>"Mail  Sent to Customer");				
			}
			else
			{
				$response_array = array("Success"=>"fail","resp"=>"Mail Not Sent");				
...
if((isset($obj->load_order_details)) == "1" && isset($obj->load_order_details))
{
	...
	if($ord_id != "")
	{
		$sql_get_order 		= " SELECT `ord_payment_id`,ord_pay_online_mode,`ord_id_show`, `ord_custid`, `ord_total`, `ord_discount`, `ord_shipping_charges`, ";
...
//			$order_id		= $row_get_order[''];
			...
			$payment_type	= $row_get_order['ord_pay_online_mode'];
			...
			$order_data 		.= '<thead>';
         	...
         	$order_data 		.= '<th style="text-align:center">Final Total</th>';
      		...
      		$order_data 		.= '</thead>';
      		$order_data 		.= '<tbody>';
          	$order_data 		.= '<tr><td style="text-align:center">'.$payment_mode.' ( '.$payment_type.' )</td>';							
...
if((isset($obj->change_order_status)) == "1" && isset($obj->change_order_status))
{
	...
	if($type == 0)
	{
		...
		if($ord_id != "")
		{
			...
			{
				$sql_update_child_order 	= "  UPDATE `tbl_cart` SET `cart_status`= '".$new_status."',`cart_modified`='".$datetime."' WHERE `cart_orderid` = '".$ord_id."'  ";
				...
				$response_array = array("Success"=>"Success","resp"=>$sql_get_orders_details);
			}
			else
			{
...
if((isset($obj->update_comments)) == "1" && isset($obj->update_comments))
{
	...
	$ord_id  = $obj->ord_id;
	$response_array = array();
	$sql_update_comment = " UPDATE `tbl_order` SET `ord_admin_comment`='".$comment."' WHERE ord_id ='".$ord_id."' ";
...
if((isset($obj->update_feedback)) == "1" && isset($obj->update_feedback))
{
    $feedback  = $obj->feedback;
	$ord_id  = $obj->ord_id;
	$response_array = array();
	$sql_update_comment = " UPDATE `tbl_order` SET `feedback`='".$feedback."' WHERE ord_id ='".$ord_id."' ";
	$res_update_comment = mysqli_query($db_con,$sql_update_comment) or die(mysqli_error($db_con));
	if($res_update_comment)
	{
		$response_array = array("Success"=>"Success","resp"=>"feedback Updated Successfully...!");
	}
	else
	{
		$response_array = array("Success"=>"fail","resp"=>"");
