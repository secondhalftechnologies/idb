...
if((isset($obj->load_spec_parts)) == "1" && isset($obj->load_spec_parts))
{
	...
	if($req_type != "")
	{
		...
		{
			$data .= '<button type="submit" name="reg_submit_add" class="btn-success">Add Packing</button>';
		}
		elseif($spec_id != "" && $req_type == "edit")
		{
			$data .= '<button type="submit" name="reg_submit_edit" class="btn-success">Update Packing</button>';
		}
		elseif($spec_id != "" && $req_type == "error")
		{
			$data .= '<button type="submit" name="reg_submit_error" class="btn-success">Update Packing</button>';
		}
...
if((isset($obj->load_spec)) == "1" && isset($obj->load_spec))
{
	...
	if($page != "" && $per_page != "")
	{
		...
		if($search_text != "")
		{
			$sql_load_data .= " AND (tc.cat_name like '%".$search_text."%' or tbm.packing_name like '%".$search_text."%' or tbm.created_date like '%".$search_text."%' or  tbm.modified_date like '%".$search_text."%')";
		}
		...
		if($result_load_data)
		{
			if(strcmp($data_count,"0") !== 0)
			{
				...
				$spec_data .= '<th style="text-align:center">Packing Value</th>';
				...
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					...
					$spec_data .= '<td style="text-align:center">'.ucwords($row_load_data['name_packing_created_by']).'</td>';
					$spec_data .= '<td style="text-align:center">'.$row_load_data['created_date'].'</td>';
					if($row_load_data['modified_by'] != '')
					{
						$spec_data .= '<td style="text-align:center">'.ucwords($row_load_data['name_packing_modified_by']).'</td>';
					}
					else
					{
