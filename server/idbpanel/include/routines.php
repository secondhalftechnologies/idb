...
if ($_SERVER['HTTP_HOST'] == "localhost" || preg_match("/^192\.168\.0.\d+$/",$_SERVER['HTTP_HOST']) || preg_match("/^praful$/",$_SERVER['HTTP_HOST'])) 
{
	...
	{
		$BaseFolder = "http://localhost/idb/server/idbpanel/";		
	}
	else
	{
		$BaseFolder = "http://192.168.0.13/idb/server/idbpanel/";	
	}
...
{	
	if($email != "" && $subject != "" || $message !="")
	{
		...
		$mail->FromName 	= 'Indian Dava Bajar';
		$mail->AddAddress($email,$email);
		$mail->AddReplyTo('support@kumar7.com', 'Indian Dava Bajar');
		$mail->WordWrap 	= 50;                             
		$mail->IsHTML(true);                              
...
function delete($table ,$where=array(), $not_where_array=array(), $and_like_array=array(), $or_like_array=array())
{
	global $db_con;
	if($table=="")
	{
