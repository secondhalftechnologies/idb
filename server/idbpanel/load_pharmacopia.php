...
if((isset($obj->load_pharmacopia_parts)) == "1" && isset($obj->load_pharmacopia_parts))
{
	...
	if($req_type != "")
	{
		...
		{
			$data .= '<button type="submit" name="reg_submit_add" class="btn-success">Add Pharmacopia</button>';			
		}
		elseif($pharmacopia_id != "" && $req_type == "edit")
		{
			$data .= '<button type="submit" name="reg_submit_edit" class="btn-success">Update Pharmacopia</button>';			
		}			
		elseif($pharmacopia_id != "" && $req_type == "error")
		{
			$data .= '<button type="submit" name="reg_submit_error" class="btn-success">Update Pharmacopia</button>';						
		}
			...
			{		
				...
				$pharmacopia_data .= '<table id="tbl_user" class="table table-bordered dataTable">';
    	 		$pharmacopia_data .= '<thead>';
    	  		$pharmacopia_data .= '<tr>';
         		$pharmacopia_data .= '<th style="text-align:center">Sr. No.</th>';
				$pharmacopia_data .= '<th style="text-align:center">Id</th>';
				$pharmacopia_data .= '<th style="text-align:center">Pharmacopia Value</th>';
				$pharmacopia_data .= '<th style="text-align:center">Created By</th>';
				$pharmacopia_data .= '<th style="text-align:center">Created</th>';
				$pharmacopia_data .= '<th style="text-align:center">Modified By</th>';
				$pharmacopia_data .= '<th style="text-align:center">Modified</th>';	
				$dis = checkFunctionalityRight("view_pharmacopia.php",3);
				if($dis)
				{					
					$pharmacopia_data .= '<th style="text-align:center">Status</th>';											
				}
				$edit = checkFunctionalityRight("view_pharmacopia.php",1);
				if($edit)
				{					
					$pharmacopia_data .= '<th style="text-align:center">Edit</th>';			
				}	
				$delete = checkFunctionalityRight("view_pharmacopia.php",2);
				if($delete)
				{					
					$pharmacopia_data .= '<th style="text-align:center"><div style="text-align:center"><input type="button"  value="Delete" onclick="multipleDelete();" class="btn-danger"/></div></th>';
				}					
          		$pharmacopia_data .= '</tr>';
      			$pharmacopia_data .= '</thead>';
				...
				{
	    		  	$pharmacopia_data .= '<tr>';				
					$pharmacopia_data .= '<td style="text-align:center">'.++$start_offset.'</td>';				
					$pharmacopia_data .= '<td style="text-align:center">'.$row_load_data['pharmacopia_id'].'</td>';
					...
					$pharmacopia_data .= '<td style="text-align:center">'.ucwords($row_load_data['name_pharmacopia_created_by']).'</td>';
					$pharmacopia_data .= '<td style="text-align:center">'.$row_load_data['pharmacopia_created'].'</td>';
					$pharmacopia_data .= '<td style="text-align:center">'.ucwords($row_load_data['name_pharmacopia_modified_by']).'</td>';
					$pharmacopia_data .= '<td style="text-align:center">'.$row_load_data['pharmacopia_modified'].'</td>';
					$dis = checkFunctionalityRight("view_pharmacopia.php",3);
					if($dis)
					{					
