	...
	if($page != "" && $per_page != "")
	{
		...
		if($result_load_data)
		{
			if(strcmp($data_count,"0") !== 0)
			{
				...
				$spec_data .= '<th style="text-align:center" style="text-align:center">Category Name</th>';
				$spec_data .= '<th style="text-align:center">Form Factor Name</th>';
				...
				while($row_load_data = mysqli_fetch_array($result_load_data))
				{
					...
					$spec_data .= '<td  style="text-align:center">'.ucwords($row_load_data['name_form_factor_created_by']).'</td>';
					$spec_data .= '<td style="text-align:center">'.$row_load_data['created_date'].'</td>';
					if($row_load_data['modified_by'] != '')
					{
						$spec_data .= '<td  style="text-align:center">'.ucwords($row_load_data['name_form_factor_modified_by']).'</td>';
					}
					else
					{
