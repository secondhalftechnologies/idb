...
if((isset($obj->load_attribute_parts)) == "1" && isset($obj->load_attribute_parts))
{
	...
	if($req_type != "")
	{
		...
		$data .= '<div class="control-group">';
		$data .= '<label for="tasktitel" class="control-label">Application Name <sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text" id="attribute_name" name="attribute_name" placeholder="Application Name" class="input-xarge" data-rule-required="true" ';
		if($spec_id != "" && $req_type == "edit")
		{
		...
		{
			$data .= '<button type="submit" name="reg_submit_add" class="btn-success">Add Application</button>';
		}
		elseif($spec_id != "" && $req_type == "edit")
		{
			$data .= '<button type="submit" name="reg_submit_edit" class="btn-success">Update Application</button>';
		}
		elseif($spec_id != "" && $req_type == "error")
		{
			$data .= '<button type="submit" name="reg_submit_error" class="btn-success">Update Application</button>';
		}
	...
	if($page != "" && $per_page != "")
	{
		...
		if($result_load_data)
		{
			if(strcmp($data_count,"0") !== 0)
			{
				...
				$spec_data .= '<th style="text-align:center">Application Name</th>';
...
if(isset($_POST['insert_req']) && $_POST['insert_req']==1)
{
	...
	if($data['attribute_name']!="" && $data['status']!="")
	{
		...
		quit('Application added successfully...!',1);
	}
	else
	{
...
if(isset($_POST['update_req']) && $_POST['update_req']==1)
{
	...
	if($data['attribute_name']!="" && $data['status']!="")
	{
		...
		quit('Application updated successfully...!',1);
	}
	else
	{
		quit('Something went wrong...!');
