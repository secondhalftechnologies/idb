...
if((isset($obj->delete_subject)) == "1" && isset($obj->delete_subject))
{
	...
	if(!empty($ar_offering_id))
	{
		foreach($ar_offering_id as $offering_id)	
		{
			...
			$result =lookup_value('tbl_class_branchoffering',array(),array("branchoff_offid"=>$offering_id),$not_where_array,$and_like_array,$or_like_array);
			while($row = mysqli_fetch_array($result))
			{
				$check = delete("tbl_class_branchoffering",array("branchoff_offid"=>$offering_id),array(),array(),array());
				$check = delete("tbl_class_offfilter",array("offfilt_offid"=>$row['branchoff_id']),array(),array(),array());
				$check = delete("tbl_class_branchoffering",array("branchoff_offid"=>$row['branchoff_id']),array(),array(),array());
			}
	...
	if($page != "" && $per_page != "")	
	{
		...
		if(strcmp($data_count,"0") !== 0)
		{
			...
			while($row_load_data = mysqli_fetch_array($result_load_data))
			{
				...
				$subject_data .= '<td class="center-text">'.$row_load_data['offering_id'].'</td>';
				$subject_data .= '<td>';
				$subject_data .= '<div class="center-text">';				
