<?php
	...
	function insert_offering($class_id,$branch_id,$off_type,$offering,$fee,$seat,$status,$duration,$duration_type,$sessions,$ts_ratio,$off_desc)
	{ 
		...
		global $db_con, $datetime,$uid;
			...
			$sql_insert_offering .=" branchoff_durationtype,branchoff_sessions , `branchoff_status`, `branchoff_created`, `branchoff_created_by`,ts_ratio,off_desc)";
			$sql_insert_offering .=" VALUES ('".$class_id."', '".$branch_id."', '".$offering."','".$off_type."', '".$seat."','".$fee."','".$duration."','".$duration_type."','".$sessions."', '".$status."',";
			$sql_insert_offering .="   '".$datetime."', '".$uid."','".$ts_ratio."','".$off_desc."')";
	...
	function insertClass($response_array,$class_name,$class_description,$class_primary_email,$class_secondary_email,$class_primary_phone,$class_secondary_phone,$class_website,$class_status,$class_bank_ifsc_code,$class_bank_account_number,$class_bank_address,$class_bank_name,$class_beneficiary_name,$class_owner,$classtype,$method,$logo,$fb_link,$tgline,$about_instructor,$establish_year,$reg_fee,$class_years ,$subjects_taught,$class_boards ,$batch_strength ,$faculty_strength)
	{
		global $uid;
		global $db_con, $datetime;
		global $obj;
		...
		{*/
			...
			$sql_insert_class .=" `class_bank_ifsc_code`,`class_bank_account_number`, `class_bank_address`,`class_bank_name`,`class_beneficiary_name`,class_owner,fb_link,tgline,about_instructor,establish_year,reg_fee,class_years ,subjects_taught,class_boards ,batch_strength,faculty_strength)";
			...
			$sql_insert_class .=" '".$class_bank_account_number."','".$class_bank_address."','".$class_bank_name."','".$class_beneficiary_name."','".$class_owner."','".$fb_link."','".$tgline."','".$about_instructor."','".$establish_year."','".$reg_fee."','".$class_years."','".$subjects_taught."','".$class_boards."','".$batch_strength."','".$faculty_strength."')";
	...
	function updateClass($response_array,$class_id,$class_name,$class_description,$class_primary_email,$class_secondary_email,$class_primary_phone,$class_secondary_phone,$class_website,$class_status,$class_bank_ifsc_code,$class_bank_account_number,$class_bank_address,$class_bank_name,$class_beneficiary_name,$class_owner,$logo,$classtype,$method,$fb_link,$tgline,$about_instructor,$establish_year,$reg_fee,$class_years ,$subjects_taught,$class_boards,$batch_strength ,$faculty_strength)
	{
		global $uid;
		global $db_con, $datetime;
	...
	//	$class_catids    = mysqli_real_escape_string($db_con,implode(',',$class_catids));
		...
		$sql_update_class .= "`class_bank_address`= '".$class_bank_address."',`class_bank_name`= '".$class_bank_name."',`class_beneficiary_name`= '".$class_beneficiary_name."',fb_link='".$fb_link."',tgline='".$tgline."',about_instructor='".$about_instructor."',establish_year='".$establish_year."',";
		$sql_update_class .= " reg_fee='".$reg_fee."',class_years='".$class_years."',subjects_taught='".$subjects_taught."',class_boards='".$class_boards."',";
		$sql_update_class .= " batch_strength='".$batch_strength."',faculty_strength='".$faculty_strength."' ";
	...
	function  update_offering($offering_id,$off_type,$offering,$duration,$fee,$seat,$status,$duration_type,$sessions,$ts_ratio,$off_desc)
	{
		global $uid;
		global $db_con, $datetime;	
		
		$sql_update_offering = " UPDATE `tbl_class_branchoffering` 
									SET `branchoff_type`='".$off_type."',	
...
/////////////////----------------Start :  Branch Part---------------------------////////////////////////////////////////////////////////////////	
	...
	if((isset($obj->load_branch_part)) != "" && isset($obj->load_branch_part))
	{
	  ...
	  if($req_type !="")
	  {
		...
		$data .= '<div id="addBranchPart">';
		$data .= '<h3 style="text-align:center">Branch 1</h3>';
		$data .='<input type="hidden" name="add_branch_req" value="1">';
		$data .='<input type="hidden" id="addBranchCount" name="addBranchCount" value="1">';
		...
		$data .= '<select onchange="getcity(this.value);" style="width:30%" id ="area" name="area1" data-rule-required="true" class ="select2-me" onChange="coupon(this.value);">';
		...
		$data .= '<div class="controls">';
		$data .= '<textarea id="address" rows="4" name="address1" class="input-xlarge" data-rule-required="true" >';
		$data .= '</textarea>';
		$data .= '</div>';	
		$data .= '</div>';
		...
		$data .= '<div class="controls">';
		$data .= '<input type="text" minlength=10 maxlength="10" id="mob_no" onKeyPress="return numsonly(event);" placeholder="Enter Mobile Number" Name="mob_no1" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<div class="controls">';
		$data .= '<input type="text"  id="land_no" onKeyPress="return numsonly(event);" placeholder="Enter Landline Number" Name="land_no1" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="controls">';
		$data .= '<input type="email" id="email"  placeholder="Enter Email" Name="email1" class="input-xlarge" />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<div class="controls">';
		$data .= '<input  value="" type="text" id="branch_location"  placeholder="Lat ,Lan" Name="branch_location1" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<select style=" width: 95%;margin-top:10px;" name="catid1[]" onchange="getsubcat(\''.$req_type.'\',1)" multiple="multiple"  id="parent_catid1" onChange="console.log($(this).children(":selected").length)" required placeholder="Type Category" class="input-block-level ">';
		...
		$data .= '<div class="control-group">';
		$data .= '<label for="tasktitel" class="control-label">Category<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';      $data .= '<div class="controls">';
		$data .= '<select style=" width: 95%;margin-top:10px;" name="catid1[]" multiple="multiple"  id="catid1" onChange="console.log($(this).children(":selected").length)" placeholder="Type Category" class="input-block-level ">';
		if($req_type=="view" || $req_type=="edit")
		{
			$cat_array =array();
		...
		$data .= '<div class="controls">';
		$data .= '<input type="radio" name="status1" value="1"  data-rule-required="true">Active';
		$data .= '<input type="radio" style="margin:10px;" name="status1" value="0"  data-rule-required="true">Inactive<br>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<button class="btn-info" type="button" onclick="addBranch()"> + </button>';
		$data .= '<button class="btn-danger" id="remove_btn" type="button" disabled="disabled" onclick="removeBranch()"> - </button>';
		$data .= '</div>';
		$data .= '</div>';
	...
	//	$data .= 'CKEDITOR.replace( "address",{height:"150", width:"100%"});';
		...
		$data .= '<label for="tasktitel" class="control-label">Mobile Number</label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text" value="'.$branch_row['class_branch_phone'].'" minlength=10 maxlength="10" id="mob_no" onKeyPress="return numsonly(event);" placeholder="Enter Mobile Number" Name="mob_no" class="input-xlarge" />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<label for="tasktitel" class="control-label">Landline Number</label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text" value="'.$branch_row['branch_landno'].'"  id="land_no" onKeyPress="return numsonly(event);" placeholder="Enter Landline Number" Name="land_no" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="controls">';
		$data .= '<input value="'.$branch_row['class_branch_email'].'" type="email" id="email"  placeholder="Enter Email" Name="email" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<label for="tasktitel" class="control-label">Email</label>';
		$data .= '<div class="controls">';
		$data .= '<input readonly="readonly" value="'.$branch_row['class_branch_email'].'" type="email" id="email"  placeholder="Enter Email" Name="email" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';	
...
//===========================Start Add BRanch=============================================//	
	if((isset($_POST['add_branch_req'])) == "1" && isset($_POST['add_branch_req']))
	{
		...
		$branchCount = $_POST['addBranchCount'] ;  
		$added_count = 0;        
		for($i=1;$i<=$branchCount;$i++)
		{
			$area		 = $_POST['area'.$i];
			$address	 = mysqli_real_escape_string($db_con,$_POST['address'.$i]);
			$mob_no		 = mysqli_real_escape_string($db_con,$_POST['mob_no'.$i]);
			$email 		 = mysqli_real_escape_string($db_con,$_POST['email'.$i]);
			$status      = $_POST['status'.$i];
			$land_no     = mysqli_real_escape_string($db_con,$_POST['land_no'.$i]);
			$class_catids = $_POST['catid'.$i];
			$branch_location = mysqli_real_escape_string($db_con,$_POST['branch_location'.$i]);
			...
			$sql_insert_branch	.= " branch_landno ,`class_branch_address`,class_branch_status,branch_location,class_branch_created,class_branch_created_by ) ";
			$sql_insert_branch .=" VALUES ('".$class_id."', '".$area."', '".$mob_no."', '".$email."','".$land_no."','".$address."', '".$status."',";
			$sql_insert_branch .=" '".$branch_location."', '".$datetime."', '".$uid."')";
		...
		if($added_count==$branchCount)
		{
			$response_array = array("Success"=>"Success","resp"=>"Branches Added Successfully",'class_id'=>$class_id);
		}
		else
		{
			$response_array = array("Success"=>"fail","resp"=>"Some Branch not Added");
		}
...
//===========================End Add BRanch=============================================//
	...
	if((isset($_POST['update_branch_req'])) == "1" && isset($_POST['update_branch_req']))
	{
		...
		$mob_no		 = mysqli_real_escape_string($db_con,$_POST['mob_no']);
		$email 		 = mysqli_real_escape_string($db_con,$_POST['email']);
		$status      = $_POST['status'];
		$class_id    = $_POST['class_id'];
		$land_no     = mysqli_real_escape_string($db_con,$_POST['land_no']);
		$class_catids = $_POST['catid'];
		$branch_location = mysqli_real_escape_string($db_con,$_POST['branch_location']);
		$response_array =array();
		     ...
		     $sql_update_branch .= " ,class_branch_address='".$address."',branch_landno ='".$land_no."',class_branch_modified='".$datetime."' ,class_branch_status='".$status."' ,branch_location='".$branch_location."',";
	  ...
	  if($req_type !="")
	  {
		if($req_type =="view" || $req_type =="edit")
		{
	  	...
	  	if($req_type =="add")
	  	{
			...
			$data .= '<div class="controls">';
			$data .= '<select style="width:70%" id="offering" name="offering[]" multiple="multiple" placeholder="Offering" onChange="console.log($(this).children(":selected").length)" data-rule-required="true" class = "select2-me offering1" onChange="coupon(this.value);">';
			$data .= '<option value="" disabled="disabled">Select Offering</option>';
			...
			$data .= '<div class="control-group">';		
			$data .= '<label for="tasktitel" class="control-label">Offering Description</label>';
			$data .= '<div class="controls">';
			$data .= '<textarea id="off_desc" style="width:90%"  placeholder="Enter Offering Description" Name="off_desc" class="input-xlarge"  ></textarea>';
			$data .= '</div>';
			$data .= '</div>';
			...
			$data .= '
					...
					<tr>
					  <th>
					     Days<br>
					     <input  type="checkbox" name="checkall" id="checkall" onClick="allDays()"> All
					  </th>
					  <th>From</th>
					  <th>To</th>
					</thead>  
					...
					<tbody>
					<tr>
					<td><input class="cdays"  type="checkbox" name="days[]" id="Monday" value="Monday"> Mon</td>
					...
					<tr>
					  <td><input class="cdays"  type="checkbox" name="days[]" id="Tuesday" value="Tuesday"> Tue</td>
					...
					<tr>
					 <td><input class="cdays"  type="checkbox" name="days[]" id="Wednesday" value="Wednesday"> Wed</td>
					...
					<tr>
					<td><input class="cdays"  type="checkbox" name="days[]" id="Thursday" value="Thursday"> Thu</td>
					...
					<tr>
					<td><input class="cdays"  type="checkbox" name="days[]" id="Friday" value="Friday"> Fri</td>
					...
					<tr>
					 <td><input class="cdays"  type="checkbox" name="days[]" id="Saturday" value="Saturday"> Sat</td>
					...
					<tr>
					 <td><input  class="cdays" type="checkbox" name="days[]" id="Sunday" value="Sunday"> Sun</td>
    ...
    $(".bootstrap-timepicker-hour").html("'.date('h').'");
	 $(".bootstrap-timepicker-minute").html("'.date('i').'");
	  $(".bootstrap-timepicker-meridian").html("'.date('A').'");
			...
			$data .= '<div class="control-group">';
			$data .= '<label for="tasktitel" class="control-label">Duration  Type<sup class="validfield"></label>';
			$data .= '<div class="controls">';
			$data .= '<select  style="width:30%" id ="add_duration_type" name="duration_type" class ="select2-me">';
			...
			$data .= '<div class="controls">';
			$data .= '<input type="text" minlength="1" maxlength="2" id="duration" onKeyPress="return numsonly(event);" placeholder="Enter Duration" Name="duration" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';	
			...
			$data .= '<div class="controls">';
			$data .= '<input type="text" minlength="1" maxlength="2" id="sessions" onKeyPress="return numsonly(event);" placeholder="Enter Session" Name="sessions" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';
			
			$data .= '<div class="control-group">';		
			$data .= '<label for="tasktitel" class="control-label">Fee in Rupees</label>';
			$data .= '<div class="controls">';
			$data .= '<input type="text" minlength="1" maxlength="6" id="fee" onKeyPress="return numsonly(event);" placeholder="Enter Fee" Name="fee" class="input-xlarge"  />';
			...
			$data .= '<div class="controls">';
			$data .= '<input type="text" minlength="1" maxlength="5" id="seat" onKeyPress="return numsonly(event);" placeholder="Enter Seat Availability" Name="seat" class="input-xlarge" />';
			$data .= '</div>';
			$data .= '</div>';	
		...
		if($req_type =="edit")
	  	{
			...
			$data .= '<div class="control-group">';
			$data .= '<label for="tasktitel" class="control-label"> Offering</label>'.$row['offering_id'];
			$data .= '<div class="controls">';
			...
			$sql_get_offring =" SELECT * FROM tbl_offering WHERE  cat_id IN (SELECT DISTINCT cat_id FROM tbl_class_cats WHERE branch_id ='".$branch_id."' )";            $sql_get_offring .=" AND offering_id NOT IN (SELECT DISTINCT branchoff_offid FROM tbl_class_branchoffering WHERE branchoff_branchid='".$branch_id."' AND offering_id !='".$off_row['branchoff_offid']."')";
			
				$res_get_offering =mysqli_query($db_con,$sql_get_offring) or die(mysqli_error($db_con));
				while($row_get_off =mysqli_fetch_array($res_get_offering))
				{
					$data .= '<option value="'.$row_get_off['offering_id'].'"  ';
					  
					      if($row_get_off['offering_id']==$off_row['branchoff_offid'])
						  {
							  $data .=' selected="selected" ';
						  }
					 
					$data .=' >'.ucwords($row_get_off['offering_name']).'</option>';
				}
			...
			$data .= '<label for="tasktitel" class="control-label">Offering Description</label>';
			$data .= '<div class="controls">';
			$data .= '<textarea id="off_desc" style="width:90%"  placeholder="Enter Offering Description" Name="off_desc" class="input-xlarge"  >'.$off_row['off_desc'].'</textarea>';
			$data .= '</div>';
			$data .= '</div>';
			
	        //$data  .=$sql_get_offring;
		    $data .= '<div class="control-group">';
			$data .= '<label for="tasktitel" class="control-label">Time<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
			$data .= '<div class="controls">';
			$data .= '
					...
					<tbody>
					<tr>
					<td><input  type="checkbox" name="days[]" id="Monday" value="Monday"> Mon</td>
					...
					<tr>
					  <td><input  type="checkbox" name="days[]" id="Tuesday" value="Tuesday"> Tue</td>
					...
					<tr>
					 <td><input  type="checkbox" name="days[]" id="Wednesday" value="Wednesday"> Wed</td>
					...
					<tr>
					<td><input  type="checkbox" name="days[]" id="Thursday" value="Thursday"> Thu</td> 
					...
					<tr>
					<td><input  type="checkbox" name="days[]" id="Friday" value="Friday"> Fri</td>
					...
					<tr>
					 <td><input  type="checkbox" name="days[]" id="Saturday" value="Saturday"> Sat</td>
					...
					<tr>
					 <td><input  type="checkbox" name="days[]" id="Sunday" value="Sunday">  Sun</td>
...
$(".timepicker").timepicker({
    ...
    $(".bootstrap-timepicker-hour").html("'.date('h').'");
	 $(".bootstrap-timepicker-minute").html("'.date('i').'");
	  $(".bootstrap-timepicker-meridian").html("'.date('A').'");
			...
			$data .= '<div class="controls">';
			$data .= '<select  style="width:30%" id ="edit_duration_type" name="duration_type"  class ="select2-me">';
			$data .= '<option value="">Select Duration Type</option>';
			...
			$data .= '<label for="tasktitel" class="control-label">Duration</label>';
			$data .= '<div class="controls">';
			$data .= '<input value="'.$off_row['branchoff_duration'].'" type="text" minlength="1" maxlength="2" id="duration" onKeyPress="return numsonly(event);" placeholder="Enter Duration" Name="duration" class="input-xlarge" />';
			$data .= '</div>';
			$data .= '</div>';
			...
			$data .= '<label for="tasktitel" class="control-label">No. of Sessions</label>';
			$data .= '<div class="controls">';
			$data .= '<input value="'.$off_row['branchoff_sessions'].'" type="text" minlength="1" maxlength="2" id="sessions" onKeyPress="return numsonly(event);" placeholder="Enter Session" Name="sessions" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';
			...
			$data .= '<div class="controls">';
			$data .= '<input value="'.$off_row['branchoff_fee'].'" type="text" minlength="1" maxlength="6" id="fee" onKeyPress="return numsonly(event);" placeholder="Enter Fee" Name="fee" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';	
			...
			$data .= '<label for="tasktitel" class="control-label">Seat Availability</label>';
			$data .= '<div class="controls">';
			$data .= '<input value="'.$off_row['branchoff_seats'].'" type="text" minlength="1" maxlength="5" id="seat" onKeyPress="return numsonly(event);" placeholder="Enter Seat Availabilyty" Name="seat" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';	
			...
			$data .= '<label for="tasktitel" class="control-label">Teacher Student Ratio</label>';
			$data .= '<div class="controls">';
			$data .= '<input  value="'.$off_row['ts_ratio'].'" type="text"  id="ts_ratio" placeholder="1:10" Name="ts_ratio" class="input-xlarge"  />';
			$data .= '</div>';
			$data .= '</div>';
           ...
           while($day_row = mysqli_fetch_array($res_get_days))
           {
			  ...
			  $data .='$("#'.$day_row['day'].'").prop("checked", true);';
			}
		...
		//	$data .= '<input type="text" class="input-xlarge" readonly="readonly"  value="'.$offering_row['class_branch_address'].'" readonly="readonly" />';
		   ...
		   $data .= '</select>'; 
			...
			$data .= '<label for="tasktitel" class="control-label">Offering Description</label>';
			$data .= '<div class="controls">';
			$data .= '<textarea  disabled="disabled" id="off_desc" style="width:90%"  placeholder="Enter Offering Description" Name="off_desc" class="input-xlarge"  >'.$off_row['off_desc'].'</textarea>';
			$data .= '</div>';
			$data .= '</div>';
	...
	if((isset($_POST['add_offering_req'])) == "1" && isset($_POST['add_offering_req']))
	{
		...
		$status	 	 = $_POST['status'];
		$days		 = $_POST['days']; //array("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday");
		$levels		 = $_POST['level'];
		$filters		 = $_POST['filters'];
		$sessions	 = $_POST['sessions'];
		$ts_ratio	 = mysqli_real_escape_string($db_con,$_POST['ts_ratio']);
		
		//echo json_encode(array("Success"=>"fail","resp"=>$days));exit();
		
		foreach($offerings as $offering)
		{
			$offering_id = insert_offering($class_id,$branch_id,$off_type,$offering,$fee,$seat,$status,$duration,$duration_type,$sessions,$ts_ratio,$off_desc);
	...
	if((isset($_POST['edit_offering_req'])) == "1" && isset($_POST['edit_offering_req']))
	{
		...
		$update_off = update_offering($offering_id,$off_type,$offering,$duration,$fee,$seat,$status,$duration_type,$sessions,$ts_ratio,$off_desc);
		if($update_off)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if((isset($obj->load_add_class_part)) != "" && isset($obj->load_add_class_part))
	{
		...
		$data .= '<div class="control-group span6">';
		$data .= '<label for="tasktitel" class="control-label">Primary Email-ID </label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text" id="pri_email" name="pri_email" placeholder="Primary Email-ID" class="input-large" data-rule-email="true"  ';
		if($class_id != "" && $req_type == "error")
		{
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Established Year</label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="establish_year" minlength="4" name="establish_year" placeholder="Established Year" class="input-large" maxlength="4" onKeyPress="return isNumberKey(event)" ';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value="'.$row_class_data->org_secondary_phone.'"';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['establish_year'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['establish_year'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Registration fee in Rs. </label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="reg_fee" name="reg_fee" placeholder="Registration fee" class="input-large"  onKeyPress="return isNumberKey(event)" ';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value="'.$row_class_data->org_secondary_phone.'"';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['reg_fee'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['reg_fee'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Year ( FY/ SY/ TY) </label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="class_years" name="class_years" placeholder="Years ( FY/ SY/ TY)" class="input-large"';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value="'.$row_class_data->org_secondary_phone.'"';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['class_years'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['class_years'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Subjects Taught </label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="subjects_taught" name="subjects_taught" placeholder="Subjects Taught " class="input-large"';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value="'.$row_class_data->org_secondary_phone.'"';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['subjects_taught'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['subjects_taught'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Boards </label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="class_boards" name="class_boards" placeholder="Boards" class="input-large"';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value="'.$row_class_data->org_secondary_phone.'"';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['class_boards'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['class_boards'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Batch Strength</label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="batch_strength" name="batch_strength" placeholder="Batch Strength" class="input-large"';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value=""';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['batch_strength'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['batch_strength'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<div class="control-group" style="clear:both;">';
		$data .= '<label for="tasktitel" class="control-label">Faculty Strength</label>';
		$data .= '<div class="controls">';
		$data .= '<input  type="text" id="faculty_strength" name="faculty_strength" placeholder="Faculty Strength" class="input-large"';
		if($class_id != "" && $req_type == "error")
		{
			$data .= 'value=""';					
		}
		elseif($class_id != "" && $req_type == "view")
		{
			$data .= 'value="'.$row_class_data['faculty_strength'].'" disabled';					
		}		
		else
		{
			$data .= 'value="'.$row_class_data['faculty_strength'].'"';					
		}
		$data .= '/>';
		$data .= '</div>';
		$data .= '</div>';
	...
	{  
					...
					$about_instructor       = mysqli_real_escape_string($db_con,$_POST['about_instructor']);
					
					$establish_year         = mysqli_real_escape_string($db_con, $_POST['establish_year']);
					$reg_fee                = mysqli_real_escape_string($db_con,$_POST['reg_fee']);
					$class_years            = mysqli_real_escape_string($db_con,$_POST['class_years']);
					$subjects_taught       	= mysqli_real_escape_string($db_con, $_POST['subjects_taught']);
					$class_boards           = mysqli_real_escape_string($db_con,$_POST['class_boards']);
					$batch_strength         = mysqli_real_escape_string($db_con,$_POST['batch_strength']);
					$faculty_strength       = mysqli_real_escape_string($db_con,$_POST['faculty_strength']);
	...
	/*$response_array = array("Success"=>"fail","resp"=>$_FILES['banner']['name'][0]."=".$class_description."=".$org_primary_phone."=".$org_indid."=".$org_status);
		...
		if($class_name != '' &&  $class_primary_phone!=''  && $class_status != "")
	   {    
			...
			$response_array = array("Success"=>"Success","resp"=>"Class Added Successfully");	
		}
			
		}
		else
		{
			$response_array = array("Success"=>"fail","resp"=>"Class Not Added Successfully");		
		}
	...
	if((isset($_POST['update_clas_request'])) == "1" && isset($_POST['update_clas_request']))
	{
		...
		if($class_name != '' && $class_primary_phone!='' )
		{
...
if((isset($obj->addBranchPart)) == "1" && isset($obj->addBranchPart))
{
        $div_id   = $obj->div_id;
		$req_type ='add';
	  	$data = '';
        $data .= '<div id="addBranchPart'.$div_id.'">';
		$data .= '<h3 style="text-align:center">Branch '.$div_id.'</h3>';
        $data .= '<div class="control-group">';
		$data .= '<label for="tasktitel" class="control-label">Select Area<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
		$data .= '<div class="controls">';
		$data .= '<select onchange="getcity(this.value);" style="width:30%" id ="area'.$div_id.'" name="area'.$div_id.'" data-rule-required="true" class ="select2-me">';
		$data .= '<option value="">Select Area</option>';
		...
		$data .= '<label for="tasktitel" class="control-label">Area Address<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
		$data .= '<div class="controls">';
		$data .= '<textarea id="address" rows="4" name="address'.$div_id.'" class="input-xlarge" data-rule-required="true" >';
		$data .= '</textarea>';
		$data .= '</div>';	
		$data .= '</div>';
		...
		$data .= '<label for="tasktitel" class="control-label">Mobile Number</label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text" minlength=10 maxlength="10" id="mob_no" onKeyPress="return numsonly(event);" placeholder="Enter Mobile Number" Name="mob_no'.$div_id.'" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<label for="tasktitel" class="control-label">Landline Number</label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text"  id="land_no" onKeyPress="return numsonly(event);" placeholder="Enter Landline Number" Name="land_no'.$div_id.'" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<label for="tasktitel" class="control-label">Email</label>';
		$data .= '<div class="controls">';
		$data .= '<input type="email" id="email"  placeholder="Enter Email" Name="email'.$div_id.'" class="input-xlarge" />';
		$data .= '</div>';
		$data .= '</div>';	
		...
		$data .= '<label for="tasktitel" class="control-label">Branch Location</label>';
		$data .= '<div class="controls">';
		$data .= '<input  value="" type="text" id="branch_location"  placeholder="Lat ,Lan" Name="branch_location'.$div_id.'" class="input-xlarge"  />';
		$data .= '</div>';
		$data .= '</div>';
		
		$data .= '<div class="control-group">';
		$data .= '<label for="tasktitel" class="control-label">Category<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';      $data .= '<div class="controls">';
		$data .= '<select style=" width: 95%;margin-top:10px;" name="catid'.$div_id.'[]" onchange="getsubcat(\''.$req_type.'\','.$div_id.')" multiple="multiple"  id="parent_catid'.$div_id.'" onChange="console.log($(this).children(":selected").length)" required placeholder="Type Category" class="input-block-level ">';
		$sql_get_cat ="SELECT * FROM tbl_coaching_category WHERE cat_status =1 AND cat_type='parent'";
		$res_get_cat =mysqli_query($db_con,$sql_get_cat) or die(mysqli_error($db_con));
		while($row_cat = mysqli_fetch_array($res_get_cat))
		{
		...
		$data .= '<div class="control-group">';
		$data .= '<label for="tasktitel" class="control-label">Category<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';      $data .= '<div class="controls">';
		$data .= '<select style=" width: 95%;margin-top:10px;" name="catid'.$div_id.'[]" multiple="multiple"  id="catid'.$div_id.'" onChange="console.log($(this).children(":selected").length)" placeholder="Type Category" class="input-block-level ">';
		if($req_type=="view" || $req_type=="edit")
		{
			$cat_array =array();
			$sql ="SELECT DISTINCT cat_id FROM tbl_class_cats WHERE class_id ='".$class_id."' AND parent != 1 ";
		...
		$data .= '<label for="tasktitel" class="control-label">Status<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
		$data .= '<div class="controls">';
		$data .= '<input type="radio" name="status'.$div_id.'" value="1"  data-rule-required="true">Active';
		$data .= '<input type="radio" style="margin:10px;" name="status'.$div_id.'" value="0"  data-rule-required="true">Inactive<br>';
		$data .= '</div>';
		$data .= '</div>';
		...
		$data .= '<script type="text/javascript">';
		$data .= '$("#area'.$div_id.'").select2();';
		$data .= '$("#catid'.$div_id.'").select2();';
		$data .= '$("#parent_catid'.$div_id.'").select2();';
	//	$data .= 'CKEDITOR.replace( "address",{height:"150", width:"100%"});';
		$data .= '</script>';
        $response_array = array("Success"=>"Success","resp"=>$data);				
	    echo json_encode($response_array);exit();
