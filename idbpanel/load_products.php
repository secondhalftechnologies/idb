...
///////////////////////// Start Done By satish 14022017  for category/////////////////////////
function show_full_path($cat_id,$array)
{
  	global $db_con;
	
	$sql_get_cat = "SELECT * FROM tbl_category WHERE cat_id ='".$cat_id."'";
	$res_get_cat = mysqli_query($db_con,$sql_get_cat) or die(mysql_error());
...
///////////////////////// End Done By satish 14022017  for category/////////////////////////
...
function insertProduct($prod_model_number,$prod_name,$prod_title,$prod_description,$prod_orgid,$prod_brandid,$prod_catid,$prod_google_product_category,$prod_min_quantity,$prod_quantity,$prod_max_quantity,$prod_content,$prod_list_price,$prod_recommended_price,$prod_meta_description,$prod_meta_tags,$prod_meta_title,$prod_status,$batch_specification,$error_specification,$batch_filters,$batch_levels)
{
	...
	if($num_rows_check_product == 0)
	{
		...
		$prod_name =trim($prod_name);
                ///////-----Start : Done By satish 01032017---------------/////
		$db_con->set_charset("utf8");
                ///////-----End: Done By satish 01032017---------------/////
...
//$prod_google_product_category		=htmlspecialchars(strtolower($row_load_products['prod_google_product_category']),ENT_HTML5);
$prod_google_product_category		=html_entity_decode($row_load_products['prod_google_product_category']);
			...
			$prod_name 				= rtrim($row_load_products['prod_name']);
			$prod_title 			= trim($row_load_products['prod_title']);
			$prod_slug 				= trim($row_load_products['prod_slug']);			
...
{	
	...
	if($req_type != "")
	{
		...
		{   $error_filter_parent=array();
			...
			/*done by monika for error description on 27-12-2016*/
				$text = trim( preg_replace( '/\s+/', ' ', $row_error_data['error_data'] ) );  
				$text = preg_replace("/(\r\n|\n|\r|\t)/i", '', $text);
				$row_prod_data	= json_decode($text);
		...
		if($req_type != "view" && $utype == "1")
		{
			...
			{	
				...
				/*done by monika for org on 27-12-2016*/
				else if($req_type == "error")
				{   
					if(strtolower($row_get_org['org_name']) == strtolower($row_prod_data->prod_orgid))
					{
						$data .= 'selected ';
					}					
				}
		...
		{
			$data .= '<select name="prod_orgid" id="prod_orgid" class="select2-me input-large" data-rule-required="true">';      //////////--------START : Done By satish 10022017---------////////////////////
		...
		if($req_type != "view")
		{
			...
			while($row_get_branch 	= mysqli_fetch_array($result_get_branch))
			{
				...
				if($req_type == "error")
				{
					if(strtolower($row_get_branch['brand_name']) == strtolower($row_prod_data->prod_brandid))
					{
						$data .= 'selected';
					}					
				}
...
if((isset($obj->load_products)) == "1" && isset($obj->load_products))
{
	...
	$campaign_section 		    = $obj->campaign_section;
	...
	{   if($campaign=="")
		...
		$sql_get_campaign_products  = "SELECT * FROM tbl_campaign_info WHERE cmp_info_campid='".$campaign."' ";
		if($campaign_section !="")
		{
		   $sql_get_campaign_products .= " AND cmp_info_id ='".$campaign_section."'";
		}
		$res_get_campaign_products = mysqli_query($db_con,$sql_get_campaign_products) or die(mysqli_error($db_con));	
		$num_get_campaign_products = mysqli_num_rows($res_get_campaign_products);	
		if($num_get_campaign_products!=0)
		{
			while($camp_row = mysqli_fetch_array($res_get_campaign_products))
			{
				$cmp_info_products .=$camp_row['cmp_info_products'].',';
			}
		}
		$cmp_info_products    =chop($cmp_info_products,',');	
	}
	...
	if($page != "" && $per_page != "")	
	{
		...
		//////////////////Start :  Done By satish 20022017 for marketing product  /////////////////
		if($cmp_info_products !="")
		{
			$sql_load_data  					.= " and prod_id IN (".$cmp_info_products.") ";
		}
		//////////////////End :  Done By satish 20022017 for marketing product  /////////////////
		if($search_text != "")
		{
		...
		{   
			...
			$sql_load_data  .=" WHERE tcpm1.path_id='".$prod_catid."' ";
		...
		if($no_desc != "")
		{
			$sql_load_data  .= " AND prod_description = '' ";
		}
...
// done by satish for add and remove category 
			 $sql_update_default_status 	= "	UPDATE `tbl_product_cats` ";
			 $sql_update_default_status 	.= " 	SET `prodcat_prod_status`='".$curr_status."' ";
			 $sql_update_default_status 	.= " WHERE `prodcat_prodid`='".$prod_id."'";
			 $res_update_default_status 	= mysqli_query($db_con,$sql_update_default_status) or die(mysqli_error($db_con));
			 
			 // Update Prod Status in prod-filter table
			 $sql_update_prod_filt	 = " UPDATE `tbl_product_filters` ";
			 $sql_update_prod_filt 	.= " 	SET `prodfilt_status`='".$curr_status."' ";
			 $sql_update_prod_filt 	.= " WHERE `prodfilt_prodid`='".$prod_id."' ";
			 $res_update_prod_filt	 = mysqli_query($db_con, $sql_update_prod_filt) or die(mysqli_error($db_con));
			 
			 // Update Prod Status in prod-specification table
			 $sql_update_prod_spec	= " UPDATE `tbl_products_specifications` ";
			 $sql_update_prod_spec 	.= " 	SET `prod_spec_status`='".$curr_status."' ";
			 $sql_update_prod_spec 	.= " WHERE `prod_spec_prodid`='".$prod_id."' ";
			 $res_update_prod_spec	= mysqli_query($db_con, $sql_update_prod_spec) or die(mysqli_error($db_con));
...
if((isset($obj->load_error)) == "1" && isset($obj->load_error))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		//	$prod_data .= '<th>Category</th>';
			...
			{  	
				/*done by monika for error description on 27-12-2016*/
				$text = trim( preg_replace( '/\s+/', ' ', $row_load_data['error_data'] ) );  
				$text = preg_replace("/(\r\n|\n|\r|\t)/i", '', $text);
				$get_prod_rec	= json_decode($text);
				/*done by monika for error description on 27-12-2016*/
...
if(isset($_POST['excel_product_upload']) && $_POST['excel_product_upload'] == "1")
{
	...
	if(strcmp($inputFileName, "")!==0)
	{
		...
		{			
			...
			$prod_name				= htmlspecialchars(trim($prod_name1), ENT_HTML5);			
			/* Product Name */			
			
			/* Product Title */			
			$prod_title_1			= trim(str_replace("'","",trim($allDataInSheet[$i]["C"])));
			$prod_title				= htmlspecialchars(trim($prod_title_1), ENT_HTML5);			
			/* Product Title */						
		...
		//  echo json_encode($num_rows_get_orgid);exit();
			...
			/* Product Meta Title */
$prod_meta_title		=trim($prod_meta_title);
			...
			if($prod_max_quantity == "")
			{
              ...
              $prod_max_quantity=$prod_quantity;
			}
			...
			/*done by monika for model no blank on 28-12-2016*/
			$errorflag = 0;
			$status123 = 0;
			/*done by monika for model no blank on 28-12-2016*/
			...
			$cross_check = $k + $errorflag + 1;
			if($arrayCount == $k || $arrayCount == $cross_check)
			{
				$status123 = 1;
			}
			else 
		...
		for ($i = 0; $i < $fileCount; $i++) 
		{
			...
			if(in_array($file_ext,$extension) && file_exists($file_tmp))			
			{
				...
				if($num_rows_get_product_id == 1)
				{
					...
					$prod_img_file_name = str_replace('/','-',$prod_img_file_name);
					
				//	echo json_encode(array('Success'=>'Success','resp'=>$prod_img_file_name));exit();
...
/////////// done by satish 20022017///////////////get_section
if((isset($obj->get_section)) == "1" && isset($obj->get_section))
{
	$camp_id = $obj->camp_id;
	$response_array = array();
	$data ='<option value="">Select Section</option>';
	$sql                    =" SELECT * FROM tbl_campaign_info WHERE cmp_info_campid ='".$camp_id."' ";
	$result_get_section	= mysqli_query($db_con,$sql) or die(mysqli_error($db_con));
	while($row =mysqli_fetch_array($result_get_section))
	{
		$data .='<option value="'.$row['cmp_info_id'].'">'.$row['cmp_info_section_name'].'</option>';
	}
	$response_array = array("Success"=>"Success",'resp'=>$data);
	echo json_encode($response_array);
}
