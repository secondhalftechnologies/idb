...
if((isset($obj->load_add_batch_part)) == "1" && isset($obj->load_add_batch_part))
{
	...
	{ 
        ...
        <sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
		$data .= '<div class="controls">';
		$data .= '<input type="text"  type="text" id="prod_handling" name="prod_handling"   class="input-large" value="'.@$row['prod_handling'].'" placeholder="Material Handling" data-rule-required="true" >' ;
		$data .= '</div>';
		$data .= '</div> <!-- Prod_handling -->';
