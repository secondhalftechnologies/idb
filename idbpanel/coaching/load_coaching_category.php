...
function insertCatogery($uid,$db_con,$cat_name,$cat_description,$cat_parent,$cat_status,$cat_meta_description,$cat_meta_tags,$cat_meta_title,$main_parent,$response_array)
{
	global $obj, $datetime;
	$cat_slug 			= strtolower(str_replace(" ","-",$cat_name)); 
	if($cat_parent == "parent")
	{
			$sql_check_sort_ava = "SELECT cat_sort_order FROM tbl_coaching_category WHERE cat_type = 'parent' ORDER by cat_id DESC LIMIT 0,1 ";			
	...
	{
			$sql_sort_order			= "SELECT cat_sort_order FROM tbl_coaching_category WHERE cat_type = '".$cat_parent."' ORDER by cat_sort_order DESC";
	...
	$sql_check_cat 		= " select * from tbl_coaching_category where cat_name = '".$cat_name."' and cat_type = '".$cat_parent."' "; 
	...
	{
		$sql_last_rec = "Select * from tbl_coaching_category order by cat_id desc LIMIT 0,1";
		...
		$sql_insert_category = " INSERT INTO `tbl_coaching_category`(`cat_id`, `cat_name`, `cat_description`, `cat_type`, ";
		$sql_insert_category .= " `cat_slug`, `cat_sort_order`, `cat_status`, ";
		$sql_insert_category .= " `cat_created`, `cat_createdby`, `cat_meta_description`, ";
		$sql_insert_category .= " `cat_meta_title`, main_parent,`cat_meta_tags`) ";
		...
		{
			$response_array = array("Success"=>"Success","resp"=>"Category Added Successfully Inserted.");	
		}
		else
		{
	...
	if(strcmp($inputFileName, "")!==0)
	{
		for($i=2;$i<=$arrayCount;$i++)
		{
			...
			if($existName=="" )
			{
				...
				{
					$sql_get_cat_type	= "SELECT cat_id, cat_type FROM tbl_coaching_category WHERE cat_name='".$cat_parent_type."'";
...
if((isset($obj->load_add_cat_part)) != "" && isset($obj->load_add_cat_part))
{
	...
	else if($cat_id != "" && $req_type == "edit")
	{
		...
		$sql_cat_data 		.= " `cat_meta_tags`, `cat_meta_description`, `cat_status` from tbl_coaching_category where cat_id = '".$cat_id."' ";
	...
	else if($cat_id != "" && $req_type == "view")
	{
		...
		$sql_cat_data 		.= " `cat_createdby`, `cat_modified`, `cat_modifiedby`,(SELECT cat_name FROM `tbl_coaching_category` tc WHERE cat_id = tc.cat_type) as parent_name FROM `tbl_coaching_category` WHERE `cat_id` = '".$cat_id."' ";
		$result_cat_data 	= mysqli_query($db_con,$sql_cat_data) or die(mysqli_error($db_con));
		$row_cat_data		= mysqli_fetch_array($result_cat_data);		
	}		
	$data = '';
		...
		if($req_type != "view")
		{
			$data .= '<select name="cat_type" id="cat_type" placeholder="Type" class="select2-me input-large" data-rule-required="true" style="width:100%" >';
			$data .= '<option value="">Select Type</option>';	
			$data .= '<option value="parent" ';
			if( $req_type == "view" || $req_type == "edit" && $row_cat_data['cat_type']=='parent')
			{
		...
		$sql_chk_isParent	= " SELECT * FROM `tbl_coaching_category` WHERE `cat_status`='1' AND `cat_name`!='none' AND cat_id !='".$cat_id."'";
		if($utype !=1)
		{
			 $sql_chk_isParent	.=" AND cat_createdby='".$uid."' ";
		}
		if($cat_id != "" && $req_type == "view" || $req_type == "edit")
		{
			$get_child           =get_child_ids($cat_id,$array=array());
			
			if(!empty($get_child))
			{
				$sql_chk_isParent	.=" AND cat_id NOT IN (".implode(',',$get_child).") ";
			}
		}
		$sql_chk_isParent	.= " ORDER BY `cat_name` ASC, `cat_name` ASC ";
	...
	echo json_encode($response_array);exit();*/
		if($num_chk_isParent != 0)
		{
			...
			{
				if($row_chk_isParent['cat_type']=='parent')
				{
					$data .='<option value="'.$row_chk_isParent['cat_id'].'" ';
					if($row_chk_isParent['cat_id']==$row_cat_data['cat_type'])
					{
						$data .=' selected="selected" ';
					}
					$data .=' >'.$row_chk_isParent['cat_name'].'</option>';
				}
				else
				{
					$demo =show_full_path($row_chk_isParent['cat_id'],$array=array());
					$cat_path=implode('>',array_reverse($demo)).'  '; 
					$data .='<option value="'.$row_chk_isParent['cat_id'].'" ';
					if($row_chk_isParent['cat_id']==$row_cat_data['cat_type'])
					{
						$data .=' selected="selected" ';
		...
		if($cat_id != "" && $req_type == "error")
		{
			...
			$dis	= checkFunctionalityRight("view_coaching_category.php",3);
			if(!$dis)
			{
...
if((isset($obj->insert_req)) == "1" && isset($obj->insert_req))
{
	...
	if($cat_parent !="parent")
	{
		$main_parent            =get_parent($cat_parent);
...
$response_array = array("Success"=>"fail","resp"=>$main_parent);	
	...
	{
		$response_array = insertCatogery($uid,$db_con,$cat_name,$cat_description,$cat_parent,$cat_status,$cat_meta_description,$cat_meta_tags,$cat_meta_title,$main_parent,$response_array);
	}
	else
	{
...
if((isset($obj->update_req)) == "1" && isset($obj->update_req))
{
	...
	if($cat_name != ""  && $cat_parent != "" && $cat_status != "")
	{
		$sql_check_cat 		= " select * from tbl_coaching_category where cat_name like '".$cat_name."' and `cat_id` != '".$cat_id."' "; 
		$result_check_cat 	= mysqli_query($db_con,$sql_check_cat) or die(mysqli_error($db_con));
		$num_rows_check_cat = mysqli_num_rows($result_check_cat);
		if($num_rows_check_cat == 0)
		{
			$sql_get_self_data 		= "Select * from tbl_coaching_category where `cat_id` = '".$cat_id."' ";
			...
			if($change_flag == 1)
			{
			    ...
			    $sql_update_category = " UPDATE `tbl_coaching_category` ";
...
{
	global $db_con;
	$sql_get_child    = " SELECT * FROM tbl_coaching_category WHERE cat_type= '".$cat_id."' ";
	...
	while($row_get_child =mysqli_fetch_array($res_get_child))
	{
		...
		{    
			...
			$sql_insert_into_category_path	= " INSERT INTO `tbl_coaching_category_path_master`(`cat_id`, `path_id`, `level`) ";
		...
		$sql_check_child   = " SELECT * FROM tbl_coaching_category WHERE cat_type= '".$row_get_child['cat_id']."' ";
...
{
	global $db_con;
	$sql_get_cat = " SELECT * FROM tbl_coaching_category WHERE cat_type ='".$cat_type."'";
	$res_get_cat = mysqli_query($db_con,$sql_get_cat) or die(mysqli_error($db_con));
	...
	if($num_get_cat > 0)
	{
		...
		{
			$sql_update_cat_status =" UPDATE tbl_coaching_category SET cat_status ='".$cat_status."' WHERE cat_id ='".$row_get_cat['cat_id']."'";
...
function delete_path_master($cat_id)
{
	...
	$sql_delete_path  = " DELETE FROM tbl_coaching_category_path_master WHERE cat_id ='".$cat_id."' or path_id ='".$cat_id."'";
	$res_delete_path  = mysqli_query($db_con,$sql_delete_path) or die(mysqli_error($db_con));

    $sql_get_cats     =  " SELECT * FROM tbl_coaching_category WHERE cat_type ='".$cat_id."' ";
...
function delete_category($cat_id)
{
	...
	$sql_delete_cat		= " DELETE FROM `tbl_coaching_category` WHERE `cat_id` = '".$cat_id."' ";
	...
	if($result_delete_cat)
	{
	    ...
	    $sql_check_child   =" SELECT cat_id FROM `tbl_coaching_category` WHERE `cat_type` = '".$cat_id."' ";
		$res_check_child   = mysqli_query($db_con,$sql_check_child) or die(mysqli_error($db_con));
		$num_check_child   = mysqli_num_rows($res_check_child);
...
if((isset($obj->change_status)) == "1" && isset($obj->change_status))
{
	...
	$status_flag = 0;
	$sql_check_cat 		= "Select * from tbl_coaching_category where `cat_id` = '".$cat_id."' ";
	$result_check_cat 	= mysqli_query($db_con,$sql_check_cat) or die(mysqli_error($db_con));
	...
	if($num_check_cat !=0)
	{
			...
			$sql_update_status 		= " UPDATE `tbl_coaching_category` SET `cat_status`= '".$curr_status."' ,`cat_modified` = '".$datetime."' , ";
...
if((isset($obj->load_cat)) == "1" && isset($obj->load_cat))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		$sql_load_data  .= " (SELECT cat_name FROM `tbl_coaching_category` WHERE cat_id = tc.cat_type) as parent_name ";				
		$sql_load_data  .= " FROM tbl_coaching_category tc WHERE 1=1 and cat_type like '".$cat_parent."' ";
		if(strcmp($utype,'1')!==0)
		{
		...
		{		
			...
			$edit = checkFunctionalityRight("view_coaching_category.php",1);
			if($edit)
			{
				$cat_data .= '<th class="center-text">Edit</th>';
			}
			$del = checkFunctionalityRight("view_coaching_category.php",2);
			if($del)
			{
			...
			while($row_load_data = mysqli_fetch_array($result_load_data))
			{
				...
				if($cat_parent == "parent")
				{
					$sql_check_parent 	= "SELECT * FROM `tbl_coaching_category` WHERE `cat_type` like '".$row_load_data['cat_id']."'";
				...
				{   
				    ...
				    $sql_check_parent 	= "SELECT * FROM `tbl_coaching_category` WHERE `cat_id` like '".$row_load_data['cat_type']."'";
					$result_check_parent 	= mysqli_query($db_con,$sql_check_parent) or die(mysqli_error($db_con));
...
if((isset($obj->change_cat_slug)) == "1" && isset($obj->change_cat_slug))
{
	...
	if($cat_id != "" && $cat_slug != "")
	{
		$sql_update_slug 	= " UPDATE `tbl_coaching_category` SET `cat_slug`= '".$cat_slug."',`cat_modified`= '".$datetime."',";
...
if((isset($obj->change_sort_order)) == "1" && isset($obj->change_sort_order))
{
	...
	$sql_check_self_order	= " SELECT * from tbl_coaching_category WHERE cat_id LIKE '".$cat_id."' ";
	...
	$sql_check_order 		= " SELECT * from tbl_coaching_category WHERE cat_sort_order LIKE '".$new_order."' and cat_type like '".$self_parent."' ";
	...
	if($num_rows_check_order > 0)
	{
		...
		if(($self_parent == "parent" && $other_parent == "parent" )|| ($self_parent == $other_parent ))
		{
			...
			{
				$sql_update_sort1 		= " UPDATE `tbl_coaching_category` SET `cat_sort_order`= '".$self_order."' ,`cat_modified` = '".$datetime."' ,`cat_modifiedby` = '".$uid."' WHERE `cat_id` like '".$other_cat."' ";	
				$result_update_sort2 	= mysqli_query($db_con,$sql_update_sort1) or die(mysqli_error($db_con));			
				$sql_update_sort2 		= " UPDATE `tbl_coaching_category` SET `cat_sort_order`= '".$new_order."' ,`cat_modified` = '".$datetime."' ,`cat_modifiedby` = '".$uid."' WHERE `cat_id` like '".$cat_id."' ";
			...
			{					
				$sql_update_sort 	= " UPDATE `tbl_coaching_category` SET `cat_sort_order`= '".$new_order."' ,`cat_modified` = '".$datetime."' ,`cat_modifiedby` = '".$uid."' WHERE `cat_id` like '".$cat_id."' ";
...
///////////////Start  done By satish 15022017 /////////////////////////////
if((isset($obj->download_excel)) == "1" && isset($obj->download_excel))
{
   include_once("xlsxwriter.class.php");
	...
	$sql_load_category  ="SELECT * FROM tbl_coaching_category WHERE cat_status = 1 AND cat_name !='none' ";
	if(strcmp($utype,'1')!==0)
		{
			$sql_load_category  .= " AND cat_createdby='".$uid."' ";
		}
	$sql_load_category .=" ORDER BY cat_name ASC";
	$res_load_category	= mysqli_query($db_con,$sql_load_category) or die(mysql_error($db_con));
	$num_load_category	= mysqli_num_rows($res_load_category);
	 
	$i=1;
	if($num_load_category != 0)
	{
		while($row_load_category = mysqli_fetch_array($res_load_category))
		{	
		    $cat_id   =$row_load_category['cat_id'];
			$cat_name =$row_load_category['cat_name'];
			 ///////////////////////// Start Done By satish 14022017 /////////////////////////
				   if($row_load_category['cat_type']=="parent")
				   {
					   $cat_path=$row_load_category['cat_name'];
				   }
				   else
				   {  
				      $cat_paths =show_full_path($row_load_category['cat_type'],$array=array());
                      $cat_path=implode('>',array_reverse($cat_paths)).' > '.$row_load_category['cat_name']; 
				   }
				   
				   
				    ///////////////////////// End Done By satish 14022017 /////////////////////////
			
			$data1 	= array($i++,$cat_name,$cat_path);
		...
		if(!file_exists("excelDownload/category".$timestamp))
		{
			mkdir("excelDownload/category".$timestamp);
		}
		
		$writer->writeToFile('excelDownload/category'.$timestamp.'/cat_sheet_'.$timestamp.'.xlsx');
		
		$response_array	= array("Success"=>"Success", "resp"=>'excelDownload/category'.$timestamp.'/cat_sheet_'.$timestamp.'.xlsx');
		
	}
	
	echo json_encode($response_array);
}
///////////////End  done By satish 15022017 /////////////////////////////
